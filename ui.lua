...
function Library.CreateWindow(titleText, subtitleText, hubIconId)
    ...
    local function createComponentsBuilder(container)
        ...
        function builder:Slider(labelPrefix, minVal, maxVal, defaultVal, suffix, callback)
            ...
            -- Large invisible hitbox over the track for easier clicking/dragging
            local sliderHitbox = Instance.new("TextButton", sliderFrame)
            sliderHitbox.Size = UDim2.new(1, -24, 0, 28)
            sliderHitbox.Position = UDim2.new(0.5, 0, 0.72, 0)
            sliderHitbox.AnchorPoint = Vector2.new(0.5, 0.5)
            sliderHitbox.BackgroundTransparency = 1
            sliderHitbox.Text = ""
            sliderHitbox.ZIndex = sliderBar.ZIndex + 2
            ...
            sliderHitbox.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or
                   input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    -- Remap input position relative to sliderBar (not hitbox)
                    local function updateFromInput(inp)
                        local fakeInput = {
                            Position = inp.Position
                        }
                        local percentage = math.clamp(
                            (fakeInput.Position.X - sliderBar.AbsolutePosition.X) / sliderBar.AbsoluteSize.X,
                            0, 1)
                        local rawVal = percentage * (maxVal - minVal) + minVal
                        if suffix == "s" or suffix == "x" then
                            currentVal = math.floor(rawVal * 10 + 0.5) / 10
